...
bool pool_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  if (cmd.fee == 10) {
    feeString = "0.1 DXC (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " DXC";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " DXC, with a fee of " << feeString << std::endl
    ...
    for (auto destination : cmd.dsts) {
      std::cout << "You are sending " << m_currency.formatAmount(destination.amount)
                << " DXC" << std::endl << "FROM: " << walletName << std::endl
...
bool pool_wallet::print_outputs_from_transaction(const std::vector<std::string>& args)
{
  ...
  for (size_t i = 0; i < ourTransaction.outputs.size(); ++i)
  {
    ...
    if (targetPubKey.key == outputPublicKey)
    {
      ...
      std::string dxc = m_currency.formatAmount(amount);

      sum += amount;

      found = true;

      logger(INFO, GREEN) << "The transaction output of " << dxc << " DXC belongs to you!";
    }
  }
  ...
  {
    std::string dxc = m_currency.formatAmount(sum);

    logger(INFO, GREEN) << "Outputs totalling " << dxc << " DXC were sent to your wallet!";
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "DecCash v" << PROJECT_VERSION << " Poolwallet" << std::endl;
