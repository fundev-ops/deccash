         ...
         Config &config)
{
    ...
    do
    {
        std::cout << InformationMsg("DecCash v"
    ...
    {
        std::cout << WarningMsg("It looks like DecCashd isn't open!")
                  << std::endl << std::endl
                  << WarningMsg("Ensure DecCashd is open and has finished "
                                "initializing.")
                  << std::endl
                  << WarningMsg("If it's still not working, try restarting "
                                "DecCashd. The daemon sometimes gets stuck.") 
                  << std::endl
                  << WarningMsg("Alternatively, perhaps DecCashd can't "
    ...
    while (true)
    {
        ...
        if (address.length() >= 102)
        {
            std::cout << WarningMsg("Address is wrong length!") << std::endl
                      << "It should be 101 characters long, but it is "
                      << address.length() << " characters long!" << std::endl;
        }
        else if (address.substr(0, 6) != "dECASH")
        {
            std::cout << WarningMsg("Invalid address! It should start with "
                                    "\"dxc\"!") << std::endl;
                                                        ...
                                                        address))
        {
            std::cout << WarningMsg("Failed to parse DXC address! Ensure you "
...
void help(bool viewWallet)
{
    std::cout << "Available commands:" << std::endl
              ...
              << "Display how much DXC you have" << std::endl
    ...
    if (viewWallet)
    {
        std::cout << InformationMsg("Please note you are using a view only "
                                    "wallet, and so cannot transfer DXC.")
                  << std::endl;
    }
    else
    {
                           ...
                           CryptoNote::INode &node)
{
    ...
    /* Couldn't get timestamp, maybe old node or DecCashd closed */
    if (blockTime != "")
    {
                         ...
                         std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    if (localHeight != remoteHeight)
    {
        std::cout << "Your DecCashd isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting DecCashd.\n"
...
ColouredMsg getPrompt(std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    return InformationMsg("[DXC " + shortName + "]: ");
}

void connectingMsg()
{
    std::cout << std::endl << "Making initial contact with DecCashd."
              << std::endl
...
void viewWalletMsg()
{
    std::cout << InformationMsg("Please remember that when using a view wallet "
              ...
              << std::endl << "This means if you received 100 DXC and then "
              << "sent 50 DXC, your balance would appear to still be 100 "
              << "DXC." << std::endl
